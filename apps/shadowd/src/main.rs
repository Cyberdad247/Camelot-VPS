...
const POLICY_VERSION: &str = "shadow-policy/0.3-sentinel";
...
pub(crate) struct ShadowSession {
    ...
    #[serde(default)]
    capability_lease: Option<CapabilityLease>,
...
fn forbidden(error: impl ToString) -> ApiError {
    api_error(StatusCode::FORBIDDEN, error.to_string())
}
...
async fn verify_ledger(store: &ShadowStore) -> Result<Option<String>, String> {
    ...
    for item in receipts {
        ...
        if expected_hash != item.receipt_hash {
            return Err(format!(
                "receipt hash mismatch at sequence {}",
                item.sequence
            ));
        }
        verify_detached_hex(
...
fn load_or_create_signer(path: &FsPath) -> Result<KeyPair, String> {
    ...
    match options.open(path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
            let raw = stdfs::read_to_string(path).map_err(|read_error| {
                format!("read concurrently-created signing key: {read_error}")
            })?;
...
fn lease_resources(session_id: Uuid, egress: &[String]) -> Vec<String> {
    let mut resources = vec![format!("shadow://{session_id}/workspace/**")];
    resources.extend(egress.iter().cloned());
    resources
}

fn lease_hash(session: &ShadowSession) -> Option<String> {
    session
        .capability_lease
        .as_ref()
        .and_then(|lease| serde_json::to_string(lease).ok())
        .map(|lease| hash_payload(&lease))
}
...
async fn authorize_capability(
    ...
    target: &str,
    parameters: Value,
) -> Result<EffectManifest, ApiError> {
    if !session.capabilities.iter().any(|value| value == capability) {
        return Err(forbidden(format!("Session does not contain capability: {capability}")));
    }
    let lease = session
        .capability_lease
        .as_ref()
        .ok_or_else(|| forbidden("Shadow session has no Sentinel capability lease"))?;
    let manifest = EffectManifest {
        manifest_id: Uuid::new_v4(),
        task_id: session.session_id,
        proposed_action: action.to_owned(),
        target_resource: target.to_owned(),
        hash: hash_payload(&parameters.to_string()),
        parameters,
        required_lease_type: capability.to_owned(),
        estimated_cost_credits: 0.0,
    };
    state
        .sentinel
        .authorize(lease, &manifest)
        .await
        .map_err(forbidden)?;
    Ok(manifest)
}
...
async fn authorize_effect(
    ...
    let capabilities = if effect.requested_capabilities.is_empty() {
        vec!["shadow.plan".to_owned()]
    } else {
        effect.requested_capabilities.clone()
    };
    for capability in capabilities {
        authorize_capability(
            ...
            json!({
                ...
                "risk": effect.risk,
                "manifestHash": effect.manifest_hash,
            }),
        )
        .await?;
    }
...
async fn recover_sessions(state: &AppState) -> Result<usize, String> {
    ...
    for mut session in sessions {
            ...
            && !matches!(session.state, SessionState::Sealed | SessionState::Aborted)
        {
            ...
            let has_seal = receipts
                .iter()
                .any(|item| item.action == "shadow.session.seal" && item.decision == "SEALED");
...
async fn health_ready(State(state): State<AppState>) -> (StatusCode, Json<Value>) {
    ...
    let sentinel = state.sentinel.ready().await;
    let ready = database.is_ok() && ledger && root && sentinel;
    (
        if ready {
            StatusCode::OK
        } else {
        ...
        Json(json!({
            ...
            "workspaceRoot": root,
            "sentinel": sentinel,
            "sentinelPublicKey": state.sentinel.public_key(),
...
async fn create_session(
    ...
    if mission.is_empty() || mission.len() > 2000 {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
            "Mission is required and must be under 2000 characters",
        ));
    }
    ...
    let resources = lease_resources(id, &allowed_egress);
        ...
        .await
    {
        Ok(lease) => lease,
        Err(error) => {
            let _ = fs::remove_dir_all(state.shadow_root.join(id.to_string())).await;
            return Err(api_error(
                StatusCode::SERVICE_UNAVAILABLE,
                format!("Sentinel lease issuance failed: {error}"),
            ));
        }
    };
    ...
    let mut session = ShadowSession {
        ...
        risk_ceiling,
        capabilities,
        capability_lease: Some(lease.clone()),
        bounds: ResourceBounds {
            memory_mb,
            cpu_quota_percent,
            ttl_seconds,
    ...
    let item = receipt(
        ...
        &json!({
            ...
            "leaseHash": hash_payload(&serde_json::to_string(&lease).unwrap_or_default()),
        }),
    )
    .await?;
    let expected = session.state_version;
...
async fn propose_effect(
    ...
    if request.risk == RiskRing::R6 && !state.allow_r6 {
        return Err(api_error(
            StatusCode::FORBIDDEN,
            "R6 effects are fail-closed until sovereign authentication is configured",
        ));
    }
    if request.risk > session.risk_ceiling {
        return Err(api_error(
            StatusCode::FORBIDDEN,
            "Requested effect exceeds session risk ceiling",
        ));
    }
    ...
    for capability in &capabilities {
            ...
            || !session.capabilities.contains(capability)
        {
            return Err(api_error(
                StatusCode::FORBIDDEN,
                format!("Capability not leased to session: {capability}"),
            ));
        }
    }
    let manifest_hash = hash_payload(
        &json!({
    ...
    if let Some(existing) = session
        .effects
        .iter()
        .find(|effect| effect.manifest_hash == manifest_hash)
    {
    ...
    authorize_effect(&state, &session, &effect).await?;

    let expected = session.state_version;
    session.state = if requires_hitl {
    ...
    let item = receipt(
        ...
        &json!({
            "effect": effect,
            "sentinelLeaseHash": lease_hash(&session),
        }),
    )
    .await?;
...
async fn approve_effect(
    ...
    if current.status == EffectStatus::Authorized {
        if current
            .approval
            .as_ref()
            .map(|approval| (approval.operator.as_str(), approval.scope.as_str()))
        ...
        return Err(api_error(
            StatusCode::CONFLICT,
            "Effect already authorized by another decision",
        ));
    }
    if current.status != EffectStatus::PendingApproval {
        return Err(api_error(
            StatusCode::CONFLICT,
            "Effect is not awaiting approval",
        ));
    }
    if current.risk == RiskRing::R6 && (!state.allow_r6 || scope != "sovereign") {
        return Err(api_error(
            StatusCode::FORBIDDEN,
            "R6 requires enabled sovereign authentication and sovereign scope",
        ));
    }
    authorize_effect(&state, &session, &current).await?;
...
async fn deny_effect(
    ...
    if current.status == EffectStatus::Denied {
        if current
            .approval
            .as_ref()
            .map(|approval| approval.operator.as_str())
            == Some(request.operator.trim())
        {
            return Ok(Json(current));
        }
        return Err(api_error(
            StatusCode::CONFLICT,
            "Effect already denied by another decision",
        ));
    }
    if current.status != EffectStatus::PendingApproval {
        return Err(api_error(
            StatusCode::CONFLICT,
            "Only effects awaiting HITL may be denied",
        ));
    }
    let expected = session.state_version;
    let effect = &mut session.effects[effect_index];
...
async fn write_shadow_file(
    ...
    live(&session)?;
    let resource = format!("shadow://{id}/workspace/{}", relative.display());
    authorize_capability(
        &state,
        &session,
        "shadow.write",
        ...
        json!({
            ...
            "contentHash": hash_payload(&request.content),
        }),
    )
    .await?;
    let path = state
...
async fn read_shadow_file(
    ...
    if metadata.len() > MAX_TEXT_FILE_BYTES as u64 {
        return Err(api_error(
            StatusCode::PAYLOAD_TOO_LARGE,
            "Shadow read exceeds 256KB limit",
        ));
    }
    let content = fs::read_to_string(&path).await.map_err(|error| {
        api_error(
            StatusCode::BAD_REQUEST,
            format!("Shadow file is not readable text: {error}"),
        )
    })?;
    Ok(Json(json!({
        "path": relative,
        "content": content,
        "contentHash": hash_payload(&content),
    })))
}
...
async fn list_receipts(
    ...
    Ok(Json(
        state.store.receipts_for(id).await.map_err(internal)?,
    ))
}
...
async fn main() {
    ...
    let sentinel_token = env::var("CAMELOT_SENTINEL_TOKEN")
        .expect("CAMELOT_SENTINEL_TOKEN must be set; Shadow refuses unsigned capability mode");
    let sentinel_url = env::var("CAMELOT_SENTINEL_URL")
    ...
    let shadow_root = PathBuf::from(
        env::var("CAMELOT_SHADOW_ROOT").unwrap_or_else(|_| "/var/lib/camelot/shadow".into()),
    );
    fs::create_dir_all(&shadow_root)
        .await
        .expect("create shadow root");
    let db_path = PathBuf::from(env::var("CAMELOT_SHADOW_DB").unwrap_or_else(|_| {
        shadow_root.join("shadow.db").to_string_lossy().into_owned()
    }));
    let receipt_path = PathBuf::from(
        env::var("CAMELOT_SHADOW_RECEIPT_MIRROR").unwrap_or_else(|_| {
            shadow_root
                .join("ledger")
                .join("receipts.jsonl")
                .to_string_lossy()
    ...
    let signer_path = PathBuf::from(
        env::var("CAMELOT_SHADOW_SIGNING_KEY").unwrap_or_else(|_| {
            shadow_root
                .join("keys")
                .join("receipt-ed25519.key")
                .to_string_lossy()
                .into_owned()
        }),
    );

    let store = ShadowStore::open(&db_path)
        .await
        .expect("open durable Shadow store");
    let signer =
        load_or_create_signer(&signer_path).expect("load persistent Shadow signing identity");
    let verified_head = verify_ledger(&store)
        .await
        .expect("Shadow receipt ledger verification failed; refusing startup");
    let sentinel = SentinelClient::connect(sentinel_url, sentinel_token)
        .await
        .expect("connect to Sentinel signed capability authority");
    let state = AppState {
        store,
    ...
    let recovered = recover_sessions(&state)
        .await
        .expect("recover durable Shadow sessions");
    ...
    let protected = Router::new()
        ...
        .route(
            "/v1/shadow/sessions/:session_id/effects/:effect_id/deny",
            post(deny_effect),
        )
        .route(
            "/v1/shadow/sessions/:session_id/files/write",
            post(write_shadow_file),
        )
        .route(
            "/v1/shadow/sessions/:session_id/files/read",
            get(read_shadow_file),
        )
        .route(
    ...
    info!(
        ...
        r6_enabled = allow_r6,
        signer = %state.signer.public_key_hex(),
        sentinel = %state.sentinel.public_key(),
        instance_id = %state.instance_id,
        "camelot-shadowd production-hardened runtime online"
    );
    axum::serve(listener, app)
        .await
        .expect("shadowd server failed");
}
