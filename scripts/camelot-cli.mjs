...
function help() {
  ...
  console.log(dim('Run `npm run camelot -- <command>` for one-shot usage, or `npm run camelot` for the interactive shell.'));
}
...
async function execute(argv, state) {
  ...
  switch (command.toLowerCase()) {
    ...
    case 'mode': {
      ...
      if (next === 'lockdown') {
        state.threatCount = Math.max(state.threatCount, 247);
        state.shieldIntegrity = 100;
        state.responseLatency = 8;
    ...
    case 'scene': {
      if (sub === 'list' || !sub) {
        scenes.forEach(([id, index, label]) => console.log(`${state.scene === id ? green('●') : dim('○')} ${gold(index)} ${label.padEnd(22)} ${dim(id)}`));
        break;
      }
      if (sub === 'go') {
        const target = resolveScene(rest[0]);
        if (!target) throw new Error(`unknown scene: ${rest[0] || '(missing)'}`);
        state.scene = target[0];
        appendLog(state, `WORLD DIRECTOR CONTEXT // ${target[2].toUpperCase()}`);
        console.log(green(`✓ Context moved to ${target[1]} ${target[2]}`));
        console.log(dim(target[3]));
        break;
      }
      if (sub === 'explain') {
        const target = resolveScene(rest[0]) || sceneInfo(state.scene);
        console.log(`${gold(target[1])} ${bold(target[2])}\n${target[3]}`);
        break;
      }
    ...
    case 'threat': {
      if (sub === 'list' || !sub) {
        threatTypes.forEach(type => console.log(`${state.threatFilter === type ? green('●') : dim('○')} ${type}`));
        break;
      }
      if (sub === 'filter') {
        const type = normalizeThreat(rest.join(' '));
        if (!threatTypes.includes(type)) throw new Error(`unknown threat type: ${type || '(missing)'}`);
        state.threatFilter = type;
        appendLog(state, `THREAT FILTER // ${type.toUpperCase()}`);
        console.log(green(`✓ Threat filter applied: ${type}`));
        break;
      }
    ...
    case 'defense': {
      if (sub === 'list' || !sub) {
        Object.entries(defenses).forEach(([id, label]) => console.log(`${state.defenses[id] ? green('● ACTIVE') : red('○ OFFLINE')}  ${label.padEnd(22)} ${dim(id)}`));
        break;
      }
      if (sub === 'all') {
        const enabled = rest[0] === 'on';
        if (!['on', 'off'].includes(rest[0])) throw new Error('defense all requires on or off');
        Object.keys(defenses).forEach(id => { state.defenses[id] = enabled; });
        appendLog(state, `ALL DEFENSE LAYERS ${enabled ? 'ARMED' : 'DISARMED'}`);
        console.log(enabled ? green('✓ All defense layers armed') : red('! All defense layers disarmed'));
        break;
      }
      if (['arm', 'disarm', 'toggle'].includes(sub)) {
        const id = rest[0];
        if (!defenses[id]) throw new Error(`unknown defense: ${id || '(missing)'}`);
        state.defenses[id] = sub === 'toggle' ? !state.defenses[id] : sub === 'arm';
        appendLog(state, `${defenses[id].toUpperCase()} ${state.defenses[id] ? 'ARMED' : 'DISARMED'}`);
        console.log(`${state.defenses[id] ? green('✓ ARMED') : red('! DISARMED')} ${defenses[id]}`);
        break;
      }
    ...
    case 'auto': {
      ...
      state.autoResponse = sub === 'on';
      appendLog(state, `AUTONOMOUS RESPONSE ${state.autoResponse ? 'ENABLED' : 'DISABLED'}`);
      console.log(green(`✓ Autonomous response ${state.autoResponse ? 'enabled' : 'disabled'}`));
      break;
    }
    case 'counter': {
      if (sub !== 'deploy') throw new Error('counter currently supports deploy');
      state.threatCount = Math.max(0, state.threatCount - 64);
      state.shieldIntegrity = Math.min(100, state.shieldIntegrity + 0.2);
      state.responseLatency = Math.max(6, state.responseLatency - 2);
      appendLog(state, 'COUNTERMEASURE SWARM DEPLOYED // 1,284 ROUTES/MIN');
      console.log(green(`✓ Countermeasure swarm deployed`));
      console.log(dim(`Threats ${state.threatCount} · Shield ${state.shieldIntegrity.toFixed(1)}% · Latency ${state.responseLatency}ms`));
      break;
    }
    case 'command': {
      const text = [sub, ...rest].filter(Boolean).join(' ').trim();
      if (!text) throw new Error('command requires text');
      appendLog(state, `COMMAND EXECUTED // ${text.toUpperCase()}`);
      console.log(green(`✓ Command recorded: ${text}`));
      break;
    }
    case 'log': {
      const count = Math.min(50, Math.max(1, Number.parseInt(sub || '10', 10) || 10));
      (state.logs || []).slice(0, count).forEach(item => console.log(`${dim(item.at)}  ${item.message}`));
      break;
    }
    case 'bifrost': {
      if (sub === 'status') {
        const result = await bifrost('/api/bifrost/config');
        console.log(JSON.stringify(result, null, 2));
        break;
      }
      if (sub === 'probe') {
        const realm = rest[0];
        if (!['multivoice', 'godseye', 'worldmonitor'].includes(realm)) throw new Error('bifrost probe requires multivoice, godseye, or worldmonitor');
        const result = await bifrost(`/api/bifrost/probe/${realm}`);
        console.log(JSON.stringify(result, null, 2));
        break;
      }
      throw new Error('bifrost supports status or probe <realm>');
    }
    case 'reset': {
      const fresh = defaultState();
      Object.keys(state).forEach(key => delete state[key]);
      Object.assign(state, fresh);
      console.log(green('✓ CLI state reset'));
      break;
    }
    case 'help':
    case '?':
...
async function interactive(state) {
  banner();
  printStatus(state);
  console.log(dim('Type `help` for commands. Arrow history is supported by your terminal.\n'));
  ...
  rl.on('line', async line => {
    ...
    try {
      const result = await execute(args, state);
...
if (argv.length) {
  try {
    const result = await execute(argv, state);
