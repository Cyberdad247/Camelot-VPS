...
import {
  Activity, Brain, Castle, ChevronRight, CircleDot, Command, Crosshair, Database,
...
const threats: { id: ThreatFilter; label: string }[] = [
  ...
  { id: 'ai-adversary', label: 'AI Adversary' },
...
export const SovereignTerminalCommandCenter: React.FC = () => {
  ...
  const executeCrossing = async () => {
    setBusy(true);
    pushLine('info', `Crossing request // ${selectedRealm.label} via ${selectedRealm.transport}`);
    try {
      const response = await fetch(`${BIFROST_URL}/api/bifrost/crossing`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          source: 'camelot', destination: selectedRealm.id, transport: selectedRealm.transport, intent, payload: '',
        }),
      });
      const body = await response.json();
  ...
  const printHelp = () => {
    [
      ...
      'scene list | scene go <id|00..08> | scene explain',
      'threat list | threat filter <type>',
      'defense list | defense arm|disarm|toggle <id> | defense all on|off',
      'auto on|off',
      'counter deploy',
      'bifrost status | bifrost probe <realm> | bifrost cross <realm>',
      'clear',
    ].forEach(text => pushLine('info', text));
  };
  ...
  const runCommand = async (raw: string) => {
    ...
    try {
      switch (root.toLowerCase()) {
        ...
        case 'status':
          pushLine('info', `MODE ${mode.toUpperCase()} // SCENE ${selectedScene.index} ${selectedScene.label} // THREATS ${threatCount} // DEFENSE ${readiness}% // SHIELD ${shield.toFixed(1)}% // LATENCY ${latency}ms`);
          break;
        case 'mode': {
        ...
        case 'scene': {
          ...
          if (sub === 'go') {
            ...
            const found = scenes.find(item => item.id === target || item.index === target);
        ...
        case 'defense': {
          if (sub === 'list') {
            (Object.keys(defenseLabels) as DefenseId[]).forEach(id => pushLine(defenses[id] ? 'success' : 'error', `${id} // ${defenseLabels[id]} // ${defenses[id] ? 'ACTIVE' : 'OFFLINE'}`));
            break;
          }
          if (sub === 'all') {
            const enabled = rest[0] === 'on';
            if (!['on', 'off'].includes(rest[0])) throw new Error('defense all requires on or off');
          ...
          if (['arm', 'disarm', 'toggle'].includes(sub)) {
            const id = rest[0] as DefenseId;
            if (!defenseLabels[id]) throw new Error(`unknown defense: ${id || '(missing)'}`);
            setDefenses(prev => {
              const enabled = sub === 'toggle' ? !prev[id] : sub === 'arm';
              pushLine(enabled ? 'success' : 'error', `${defenseLabels[id]} ${enabled ? 'ARMED' : 'DISARMED'}`);
        ...
        case 'auto':
          if (!['on', 'off'].includes(sub)) throw new Error('auto requires on or off');
          setAutoResponse(sub === 'on'); pushLine('success', `Autonomous response ${sub === 'on' ? 'ENABLED' : 'DISABLED'}`); break;
        case 'counter':
          if (sub !== 'deploy') throw new Error('counter currently supports deploy');
          deployCountermeasures(); break;
        case 'bifrost': {
          if (sub === 'status') {
            const response = await fetch(`${BIFROST_URL}/api/bifrost/config`); const body = await response.json();
            pushLine(response.ok ? 'success' : 'error', JSON.stringify(body)); break;
          }
          if (sub === 'probe') {
            const target = rest[0] as RealmId;
            if (!realms.some(item => item.id === target)) throw new Error('realm must be multivoice, godseye, or worldmonitor');
            await probeRealm(target); break;
          }
          if (sub === 'cross') {
            const target = rest[0] as RealmId;
            if (!realms.some(item => item.id === target)) throw new Error('realm must be multivoice, godseye, or worldmonitor');
            setRealm(target); await executeCrossing(); break;
          }
  ...
  const keyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'ArrowDown') {
      ...
      setHistoryIndex(next); setInput(next === -1 ? '' : history[next] ?? '');
    }
  };

  const quickCommands = useMemo(() => [
    'status', 'mode lockdown', 'scene list', 'defense list', 'counter deploy', 'bifrost status',
  ], []);

  return (
    <section className="sovereign-terminal" id="sovereign-terminal-command-center" onKeyDown={event => {
      ...
      <header className="st-topbar">
        ...
        <div className="st-mode-switch" aria-label="Battle state">
          <button className={mode === 'nominal' ? 'active nominal' : ''} onClick={() => applyMode('nominal')}><Power size={14}/>Nominal</button>
          <button className={mode === 'battle' ? 'active battle' : ''} onClick={() => applyMode('battle')}><Swords size={14}/>Battle</button>
          <button className={mode === 'lockdown' ? 'active lockdown' : ''} onClick={() => applyMode('lockdown')}><LockKeyhole size={14}/>Lockdown</button>
        </div>
        <div className="st-runtime"><i className="online"/><span>Browser UI</span><b>{readiness}% DEFENSE</b></div>
      </header>

      <div className="st-mobile-tabs" role="tablist">
        {(['terminal', 'operations', 'bifrost'] as const).map(tab => <button key={tab} className={activeTab === tab ? 'active' : ''} onClick={() => setActiveTab(tab)}>{tab}</button>)}
      </div>

      <div className="st-workspace">
        <aside className={`st-left ${activeTab === 'operations' ? 'mobile-active' : ''}`}>
          <div className="st-panel-heading"><Network size={14}/><span>WORLD DIRECTOR</span><b>{selectedScene.index}/08</b></div>
          <div className="st-scenes">
            {scenes.map(item => {
              const Icon = item.icon;
              return <button key={item.id} className={scene === item.id ? 'active' : ''} onClick={() => { setScene(item.id); pushLine('info', `Scene selected // ${item.index} ${item.label}`); }}>
                <span className="st-scene-index">{item.index}</span><Icon size={14}/><span><strong>{item.label}</strong><small>{item.purpose}</small></span><ChevronRight size={13}/>
              </button>;
            })}
          </div>
          <div className="st-mini-status">
            <div><span>Threats</span><b>{threatCount}</b></div><div><span>Shield</span><b>{shield.toFixed(1)}%</b></div><div><span>Latency</span><b>{latency}ms</b></div><div><span>Auto</span><b>{autoResponse ? 'ON' : 'OFF'}</b></div>
          </div>
        </aside>

        <main className={`st-center ${activeTab === 'terminal' ? 'mobile-active' : ''}`}>
          <div className="st-terminal-toolbar">
            <div><Terminal size={15}/><strong>camelot@world-tree</strong><span>~/sovereign</span></div>
            <button onClick={() => setLines([])}>CLEAR</button>
          </div>
          <div className="st-terminal-output" ref={terminalRef} role="log" aria-live="polite">
            {lines.map(line => <div key={line.id} className={`st-line ${line.kind}`}><span className="st-prompt-mark">{line.kind === 'command' ? '›' : line.kind === 'success' ? '✓' : line.kind === 'error' ? '!' : '·'}</span><pre>{line.text}</pre></div>)}
          </div>
          <div className="st-quick-commands">{quickCommands.map(command => <button key={command} onClick={() => void runCommand(command)}>{command}</button>)}</div>
          <form className="st-command-line" onSubmit={submit}>
            <span>camelot ❯</span>
            <input ref={inputRef} value={input} onChange={event => setInput(event.target.value)} onKeyDown={keyDown} placeholder="Enter sovereign command..." autoComplete="off" spellCheck={false}/>
            <button type="submit" aria-label="Execute command"><Send size={15}/></button>
          </form>
        ...
        <aside className={`st-right ${activeTab === 'bifrost' ? 'mobile-active' : ''}`}>
          <section className="st-card">
            <div className="st-panel-heading"><Shield size={14}/><span>DEFENSE GRID</span><b>{activeDefenses}/7</b></div>
            <div className="st-defense-list">
              {(Object.keys(defenseLabels) as DefenseId[]).map(id => <button key={id} className={defenses[id] ? 'active' : ''} onClick={() => toggleDefense(id)}><i/><span>{defenseLabels[id]}</span><b>{defenses[id] ? 'ARMED' : 'OFF'}</b></button>)}
            </div>
          </section>

          <section className="st-card">
            <div className="st-panel-heading"><Crosshair size={14}/><span>THREAT FILTER</span><b>{threatCount}</b></div>
            <div className="st-threat-grid">{threats.map(item => <button key={item.id} className={threatFilter === item.id ? 'active' : ''} onClick={() => { setThreatFilter(item.id); pushLine('success', `Threat filter applied // ${item.label}`); }}>{item.label}</button>)}</div>
            <button className="st-deploy" onClick={deployCountermeasures}><Activity size={14}/>DEPLOY COUNTERMEASURES</button>
          </section>

          <section className="st-card st-bifrost-card">
            <div className="st-panel-heading"><Route size={14}/><span>BIFROST</span><b>GOVERNED</b></div>
            <div className="st-realm-tabs">
              {realms.map(item => {
                const Icon = item.icon;
                return <button key={item.id} className={realm === item.id ? 'active' : ''} onClick={() => setRealm(item.id)}><Icon size={13}/><span>{item.label}</span><i className={realmState[item.id]}/></button>;
              })}
            </div>
            <label className="st-intent"><span>Crossing intent</span><textarea value={intent} onChange={event => setIntent(event.target.value)} rows={3}/></label>
            <div className="st-bifrost-actions"><button onClick={() => void probeRealm(realm)} disabled={busy}><CircleDot size={13}/>Probe</button><button className="primary" onClick={() => void executeCrossing()} disabled={busy}><Route size={13}/>{busy ? 'Crossing...' : 'Authorize Crossing'}</button></div>
          </section>
        </aside>
      </div>

      <footer className="st-footer">
        <span><Castle size={12}/>MemCastle <b>{defenses.memcastle ? 'FORTIFIED' : 'OPEN'}</b></span>
        <span><Brain size={12}/>Twin Brains <b>SYNC</b></span>
        <span><Zap size={12}/>Ouroboros <b>RECURRENT</b></span>
        <span><Database size={12}/>VFS <b>{defenses.vfs ? 'GUARDED' : 'OPEN'}</b></span>
        <span><Radio size={12}/>Bifrost <b>{selectedRealm.transport.toUpperCase()}</b></span>
      </footer>
    </section>
  );
};
