...
const CamelotCommandCenter = lazy(() => import('./components/CamelotCommandCenter').then(m => ({ default: m.CamelotCommandCenter })));
...
export default function App() {
  ...
  const [laws] = useState<SovereignLaw[]>(SOVEREIGN_LAWS);
  const [missions, setMissions] = useState<AgentMission[]>(INITIAL_MISSIONS);
  const [receipts] = useState<LedgerReceipt[]>(INITIAL_LEDGER_RECEIPTS);
  ...
  const tabLabels: Record<string, { label: string; icon: any }> = {
    command_center: { label: 'Sovereign Command Center', icon: TreeDeciduous },
  ...
  const showToast = (msg: string) => {
    setToastMessage(msg);
    window.setTimeout(() => setToastMessage(cur => (cur === msg ? null : cur)), 3500);
  };

  const toggleTabMinimize = (tabId: string = activeTab) => {
    setMinimizedTabs(prev => ({ ...prev, [tabId]: !prev[tabId] }));
  ...
  const handleRunCustomCommand = (cmd: string) => {
    ...
    if (lower.startsWith('systemctl status') || lower.startsWith('systemctl restart')) {
      ...
      window.setTimeout(() => addLog(`● ${cmd.split(' ')[2] || 'service'} - Active: active (running) [cgroup: 7.2GB cap]`, 'success'), 300);
      return;
    }

    if (lower.includes('z3') || lower.includes('verify')) {
      addLog('[Z3 SMT SOLVER]: Checking 44 formal invariants against memory bounds...', 'z3');
      window.setTimeout(() => addLog('[Z3 SMT SOLVER]: SAT: All 44 theorems proved with 0 counterexamples.', 'success'), 500);
      return;
    }
    ...
    if (lower.includes('drop_caches') || lower.includes('optimize')) {
      ...
      window.setTimeout(() => {
        addLog('[KERNEL]: Reclaimed 380MB RAM under Scarcity Protocol.', 'success');
    ...
    if (lower.includes('scan-threats')) {
      addLog('[AEGIS]: Threat scan complete. Countermeasure routes refreshed.', 'success');
      showToast('Threat matrix refreshed');
      return;
    }

    if (lower.startsWith('council --knight')) {
      addLog(`[THRONE ROOM]: ${cmd}`, 'sovereign');
      return;
    }

    window.setTimeout(() => addLog(`[KERNEL]: ${cmd} executed successfully. exit code: 0`, 'info'), 250);
  };
  ...
  const handleRestartService = (serviceId: string) => {
    ...
    setServices(prev => prev.map(service => (service.id === serviceId ? { ...service, status: 'booting' } : service)));
    addLog(`[SYSTEMD]: Restarting ${svc?.unitName || serviceId}...`, 'info');
    window.setTimeout(() => {
      setServices(prev => prev.map(service => (service.id === serviceId ? { ...service, status: 'active' } : service)));
  ...
  const handleGoLive = () => {
    ...
    showToast('//GO_LIVE executed: Live routing active');
  };

  const handleDispatch = () => {
    addLog('//DISPATCH sent to engineering nodes.', 'sovereign');
    showToast('//DISPATCH broadcast to engineering nodes');
  };

  const handleRunMission = () => {
    setActiveTab('mission');
    showToast('Navigated to Mission Arena');
  };
  ...
  const immersiveCommandCenter = activeTab === 'command_center';
  ...
  return (
    <div className="min-h-screen bg-[#040711] text-slate-200 font-sans flex flex-col selection:bg-cyan-500 selection:text-black scanlines">
      ...
      <main className={`flex-1 w-full ${(immersiveCommandCenter || activeTab === 'vps_init' || activeTab === 'docs' || activeTab === 'deck' || activeTab === 'operator') ? 'p-0' : 'p-2 sm:p-4'}`}>
        <Suspense fallback={<div className="flex-1 flex flex-col items-center justify-center min-h-[500px]"><div className="w-12 h-12 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin"/><div className="mt-4 text-cyan-500 font-mono text-sm tracking-widest font-bold animate-pulse">INITIATING DMA TRANSFER...</div></div>}>
          {isCurrentTabMinimized ? (
            <div className="max-w-4xl mx-auto my-12 p-8 bg-[#0a1020]/90 border-2 border-cyan-500/40 rounded-2xl text-center space-y-4 shadow-2xl">
              <div className="w-12 h-12 mx-auto rounded-full bg-cyan-500/10 border border-cyan-400 flex items-center justify-center text-cyan-300"><Minus className="w-6 h-6"/></div>
              <h2 className="text-xl font-bold text-white">{tabLabels[activeTab]?.label || activeTab} is Currently Minimized</h2>
              <button onClick={() => toggleTabMinimize(activeTab)} className="px-4 py-2 rounded-xl bg-cyan-500 text-black font-bold text-xs font-mono">Restore {tabLabels[activeTab]?.label}</button>
            </div>
          ) : (
            <>
              {activeTab === 'command_center' && <CamelotCommandCenter vitals={vitals} logs={terminalLogs} onExecuteCommand={handleRunCustomCommand} onNavigateTab={setActiveTab}/>} 
              {activeTab === 'vps_init' && <VpsHubInitiationConsole onExecuteCommand={handleRunCustomCommand} onNavigateTab={setActiveTab}/>} 
              {activeTab === 'docs' && <DocumentationForge onNavigateTab={setActiveTab} onExecuteCommand={handleRunCustomCommand}/>} 
              {activeTab === 'deck' && <MasterWorldTreeDeck vitals={vitals} onNavigateTab={setActiveTab} onExecuteCommand={handleRunCustomCommand}/>} 
              {activeTab === 'operator' && <OperatorConsoleHtmx onExecuteCommand={handleRunCustomCommand} onNavigateTab={setActiveTab}/>} 
              {activeTab === 'bento' && <BentoGridOverview vitals={vitals} services={services} phases={phases} laws={laws} logs={terminalLogs} onNavigateTab={setActiveTab} onGoLive={handleGoLive} onRunMission={handleRunMission} onRestartService={handleRestartService} onRunVitalsCheck={() => handleRunCustomCommand('camelot-diag --z3-verify')} onExecuteCommand={handleRunCustomCommand} onOpenBootstrapScript={() => setActiveTab('script')}/>} 
              {activeTab === 'terminal' && <BootstrapTerminal phases={phases} onExecutePhase={handleExecutePhase} onExecuteAll={handleExecuteAll} onReset={handleReset} isExecutingAll={isExecutingAll} activePhaseId={activePhaseId} terminalLogs={terminalLogs} onRunCustomCommand={handleRunCustomCommand} vpsIp={vitals.targetHost}/>} 
              {activeTab === 'vkg' && <VKGHud services={services} vitals={vitals} onRestartService={handleRestartService} onExecuteCommand={handleRunCustomCommand} onOpenOperatorConsole={() => setActiveTab('operator')} onOpenVpsInitiation={() => setActiveTab('vps_init')}/>} 
              {activeTab === 'mission' && <MissionArena missions={missions} onDispatchMission={handleDispatchAgentMission} receipts={receipts} vitals={vitals}/>} 
              {activeTab === 'laws' && <SovereignLaws laws={laws} receipts={receipts} onRunZ3Check={() => handleRunCustomCommand('z3-solver --verify-all-laws')}/>} 
              {activeTab === 'scarcity' && <ScarcityProtocol vitals={vitals} services={services} onOptimize={() => handleRunCustomCommand('optimize-memory')}/>} 
              {activeTab === 'script' && <MasterBootstrapScript scriptText={RAW_BOOTSTRAP_PROMPT} onCopy={handleCopyScript} copied={copiedScript}/>} 
            </>
          )}
        </Suspense>
      </main>

      {!immersiveCommandCenter && Object.entries(minimizedTabs).some(([, isMin]) => isMin) && (
        <div className="fixed bottom-12 right-6 z-40 bg-black/90 border border-cyan-500/50 rounded-xl px-3 py-2 flex items-center gap-2 shadow-2xl backdrop-blur-md">
          ...
          {Object.entries(minimizedTabs).map(([tabKey, isMin]) => {
            ...
            return <button key={tabKey} onClick={() => { setActiveTab(tabKey); toggleTabMinimize(tabKey); }} className="flex items-center gap-1 px-2 py-1 rounded bg-cyan-950/80 border border-cyan-500/40 text-[10px] font-mono text-cyan-300"><Icon className="w-3 h-3"/><span>{meta?.label.split(' ')[0]}</span><Plus className="w-2.5 h-2.5"/></button>;
          })}
        </div>
      )}

      {toastMessage && <div className="fixed bottom-5 right-5 z-50 px-4 py-2.5 rounded-xl bg-slate-900/95 border-2 border-cyan-400 text-cyan-200 font-mono text-xs shadow-[0_0_25px_rgba(34,211,238,0.4)] flex items-center gap-2.5"><span className="w-2 h-2 rounded-full bg-cyan-400 animate-ping"/><span>{toastMessage}</span></div>}
      ...
      {!immersiveCommandCenter && (
        <footer className="border-t border-cyan-950/80 bg-[#03060e] px-4 py-2 text-[11px] font-mono text-slate-500 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3"><span className="text-cyan-400 font-bold">CAMELOT-OS vMAX OMEGA TITAN</span><span>•</span><span>BAREMETAL HUB</span><span>•</span><span className="text-emerald-400">cgroups v2 ACTIVE</span></div>
          <div className="flex items-center gap-4 text-slate-400"><span>WASI: STRICT SANDBOX</span><span>•</span><span>Z3 THEOREMS: 44/44 SAT</span><span>•</span><span className="text-amber-400 font-bold">8GB SCARCITY PROTOCOL</span></div>
        </footer>
      )}
    </div>
  );
